...
public class EvalMacroFunctions extends AbstractFunction {
  ...
  public Object childEvaluate(
    ...
    final var mtResolver = (MapToolVariableResolver) resolver;
    // execMacro has new variable scope where as evalMacro does not.
    if (functionName.equalsIgnoreCase("execMacro")) {
      return execMacro(mtResolver.getTokenInContext(), parameters.get(0).toString());
  ...
  public static Object evalMacro(@Nonnull MapToolVariableResolver res, String line)
    ...
    String ret = MapTool.getParser().parseLine(res, res.getTokenInContext(), line, context);
