  ...
  private void renderWorld(
    ...
    for (var instruction : instructions) {
      ...
      if (token.isSnapToScale()) {
        ...
        if (token.getShape() == Token.TokenShape.FIGURE) {
          at.scale(
              (double) footprintBounds.width / image.getWidth(),
              (double) footprintBounds.width / image.getWidth());
        } else {
          at.scale(
              (double) footprintBounds.width / image.getWidth(),
      ...
      //            if (instruction.distanceTravaledToShow() != null) {
      //                String distance =
      // NumberFormat.getInstance().format(instruction.distanceTravaledToShow());
      //                renderer.delayRendering(new LabelRenderer(renderer, distance, labelX,
      // labelY));
      //                labelY += 20;
      //            }
      //            if (instruction.playerName() != null) {
      //                renderer.delayRendering(
      //                        new LabelRenderer(renderer, instruction.playerName(), labelX,
      // labelY));
      //            }
