...
public class LightingComposite implements Composite {
  ...
  public static final Composite OverlaidLights =
      new LightingComposite(new ConstrainedInterpolationBlender());
   ...
   * and a multiple of the bottom component controlled by {@link #MAX_DARKNESS_BOOST_AS_PER_128}.
   * The exact result is determined by using the top component ({@code srcC}) to interpolate between
   * the two bounds.
   *
   ...
   * #MAX_DARKNESS_BOOST_AS_PER_128} - the higher that value, the lower the transition point.
   *
   ...
   * #MAX_DARKNESS_BOOST_AS_PER_128} is set to 1. It also has a much looser relation to the soft
   ...
   * </ul>
   */
  private static final class ConstrainedInterpolationBlender implements Blender {
    private static final int MAX_DARKNESS_BOOST_AS_PER_128 = 192;
    ...
    public void blendRow(int[] dstPixels, int[] srcPixels, int[] outPixels, int samples) {
      for (int x = 0; x < samples; ++x) {
        ...
        for (int shift = 0; shift < 24; shift += 8) {
          ...
          final var resultC =
              dstC + srcC * Math.min(MAX_DARKNESS_BOOST_AS_PER_128 * dstC / 128, 255 - dstC) / 255;
