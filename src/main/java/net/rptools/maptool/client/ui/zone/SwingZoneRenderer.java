...
import net.rptools.maptool.client.ui.zone.rendering.RenderLayerManager;
 ...
 *  implementation.
 */
public class SwingZoneRenderer extends JComponent
    implements ZoneRenderer, DropTargetListener {

  private static final long serialVersionUID = 3832897780066104884L;
  private static final Logger log = LogManager.getLogger(SwingZoneRenderer.class);
   ...
   */
  public SwingZoneRenderer(Zone zone) {
  ...
  private void setViewOffset(int x, int y) {
  ...
  /**
   * This method clears {@link #visibleScreenArea},
   ...
   * #paintComponent(java.awt.Graphics)} calls this method, then adds the two optional strings, "Map not
   * visible to players" and "Player View" as appropriate.
   *
   * @param g2d Graphics2D object normally passed in by {@link #paintComponent(java.awt.Graphics)}
  ...
  public void renderZone(Graphics2D g2d, PlayerView view) {
    ...
    if (Layer.TOKEN.isEnabled()) {
      ...
      if (view.isGMView() && Layer.GM.isEnabled()) {
    ...
    if (Layer.TOKEN.isEnabled() && view.isGMView() && AppState.isShowLightSources()) {
  ...
  private void renderDrawableOverlay(
  ...
  private void renderBoard(Graphics2D g, PlayerView view) {
    Dimension size = getSize();
    if (backbuffer == null
  ...
  private void renderGrid(Graphics2D g, PlayerView view) {
    int gridSize = (int) (zone.getGrid().getSize() * getScale());
  ...
  private void renderCoordinates(Graphics2D g, PlayerView view) {
  ...
  private void showBlockedMoves(Graphics2D g, PlayerView view, Set<SelectionSet> movementSet) {
    ...
    for (SelectionSet set : movementSet) {
      ...
      if (keyToken.getLayer() == Layer.GM && !view.isGMView()) {
        continue;
      }
      ...
      for (GUID tokenGUID : set.getTokens()) {
        ...
        if (token.hasFacing() && token.getShape() == TokenShape.TOP_DOWN) {
          at.rotate(
  ...
  public Layer getActiveLayer() {
    return activeLayer != null ? activeLayer : Layer.TOKEN;
  }
  ...
  private Shape getFigureFacingArrow(int angle, int size) {
  ...
  // TODO: I don't like this hardwiring
  private Shape getCircleFacingArrow(int angle, int size) {
    int base = (int) (size * .75);
    int width = (int) (size * .35);
  ...
  // TODO: I don't like this hardwiring
  private Shape getSquareFacingArrow(int angle, int size) {
    int base = (int) (size * .75);
    int width = (int) (size * .35);
  ...
  private void renderTokens(Graphics2D g, List<Token> tokenList, PlayerView view) {
  ...
  private void renderTokens(
    ...
    for (Token token : tokenList) {
      if (token.getShape() != TokenShape.FIGURE && figuresOnly && !token.isAlwaysVisible()) {
        continue;
      }
      ...
      if (calculateStacks) {
        ...
        for (TokenLocation currLocation : getTokenLocations(Layer.TOKEN)) {
      ...
      if (!isGMView && zoneView.isUsingVision() && (token.getShape() == TokenShape.FIGURE)) {
      ...
      if (token.hasFacing()) {
        TokenShape tokenType = token.getShape();
        switch (tokenType) {
          case FIGURE:
    ...
    for (Token token : tokenPostProcessing) {
      ...
      if (isSelected) {
        ...
        if (token.hasFacing()
            && (token.getShape() == TokenShape.TOP_DOWN || token.isStamp())) {
