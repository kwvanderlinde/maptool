...
public class VisionBlockingSet {
   ...
   *
   * @param segment The vision blocking segment to add
   */
  public void add(LineSegment segment) {
    ...
    start.startsWall(segment);
    end.endsWall(segment);

    final var isOpen =
        Orientation.COUNTERCLOCKWISE == initialRay.orientationIndex(segment.p1)
            && Orientation.COUNTERCLOCKWISE != initialRay.orientationIndex(segment.p0);
    if (isOpen) {
  ...
  public @Nullable Coordinate[] solve(Envelope visionBounds) {
    ...
    for (int i = 1; i < envelopeCoordinates.length; ++i) {
      this.add(new LineSegment(envelopeCoordinates[i - 1], envelopeCoordinates[i]));
    }
