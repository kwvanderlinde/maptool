...
public final class WallTopology implements Topology {
   ...
   */
  private record VertexInternal(GUID id, int zIndex, Point2D position) {}
  ...
  // Vertices are identified by GUID. Attached data is maintained in vertexDataById
  private final Graph<GUID, WallInternal> graph;
  private final Map<GUID, VertexInternal> vertexDataById = new LinkedHashMap<>();
  ...
  private VertexInternal createVertexImpl(GUID id) throws GraphException {
    ...
    var data = new VertexInternal(id, ++nextZIndex, new Point2D.Double());
    var previous = vertexDataById.put(id, data);
  ...
  public Stream<Vertex> getVertices() {
    return this.graph.vertexSet().stream()
        .map(
            id -> {
              var vertexInternal = vertexDataById.get(id);
              return new Vertex(id, vertexInternal);
            });
  }
    ...
    return this.graph.edgeSet().stream()
        .map(
            wall -> {
              ...
              var toId = wall.to();
              var fromInternal = vertexDataById.get(fromId);
              var toInternal = vertexDataById.get(toId);
  ...
  public void removeVertex(Vertex vertex) {
    ...
    vertexDataById.remove(vertex.id());
  ...
  public Vertex merge(Vertex first, Vertex second) {
    ...
    // A copy is essential since graph.edgesOf() return a live map.
    ...
    for (var oldWall : incidentWalls) {
      ...
      if (existingWall == null) {
        ...
        // TODO Whenn walls have data to merge, do it here, accounting for whether the walls point
  ...
  private void removeDanglingVertices() {
    ...
    for (var vertexId : verticesToRemove) {
      vertexDataById.remove(vertexId);
    }
  }
  ...
  public WallTopologyDto toDto() {
    ...
    for (var vertex : this.vertexDataById.values()) {
      builder.addVertices(
